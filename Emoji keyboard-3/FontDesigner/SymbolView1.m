...
{    
    ...
    if (self) {
        ...
        self.backgroundColor = [UIColor colorFromHex:TableViewBKColor];
...
- (void)tilePages
{
    ...
    for (int index = firstNeededPageIndex; index <= lastNeededPageIndex; index++) {
        if (![self isDisplayingPageForIndex:index]) {
            ...
            if (page == nil) {
                page = [[[LablesView alloc] init] autorelease];
                ...
                for (int i=0; i<countOnePage; i++)
                {
                    ...
                    if (currentIndex == 6)
                    {
                        label.font = [UIFont fontWithName:@"Apple Color Emoji" size:26];
                    }
                    else
                    {
...
- (void)addSegment
{
    ...
    segmentControl.frame = CGRectMake(2, topMargin, 316, 40);
